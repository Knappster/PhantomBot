...
public final class ConsoleEventHandler implements Listener {
    ...
    public void onConsoleInput(ConsoleInputEvent event) throws JSONException {
        ...
        /**
         * @consolecommand channelpointstest - Sends a fake Channel Points redemption
         *                 for testing.
         */
        ...
        if (message.equalsIgnoreCase("streamlabssetup")) {
            ...
            String streamLabsClientId = com.gmt2001.Console.in.readLine().trim();

            System.out.println("");
            System.out.print(
                    "From the StreamLabs Application Settings page, please paste or enter your StreamLabs Client Secret: ");
            String streamLabsClientSecret = com.gmt2001.Console.in.readLine().trim();

            System.out.println("");
            System.out.print(
                    "From the StreamLabs Application Settings page, please paste or enter your StreamLabs Redirect URI: ");
            String streamLabsRedirectURI = com.gmt2001.Console.in.readLine().trim();
            ...
            System.out.println("");
            System.out.println("https://streamlabs.com/api/v2.0/authorize?client_id=" + streamLabsClientId
                    + "&redirect_uri=" + streamLabsRedirectURI + "&response_type=code&scope=donations.read");
            System.out.println("");
            System.out.println(
                    "Please paste or enter the access code from the URL in your browser's address bar. You can also just paste the entire URL: ");
            String streamLabsKickback = com.gmt2001.Console.in.readLine().trim();

            if (streamLabsKickback.contains("code=")) {
                streamLabsKickback = streamLabsKickback.substring(streamLabsKickback.indexOf("code=") + 5);
            }

            if (streamLabsKickback.contains("&")) {
                streamLabsKickback = streamLabsKickback.substring(0, streamLabsKickback.indexOf('&'));
            }

            Map<String, String> postData = new HashMap<>();
            postData.put("grant_type", "authorization_code");
            postData.put("client_id", streamLabsClientId);
            postData.put("client_secret", streamLabsClientSecret);
            postData.put("redirect_uri", streamLabsRedirectURI);
            postData.put("code", streamLabsKickback);

            HttpClientResponse response = HttpClient.post(
                    URI.create("https://streamlabs.com/api/v2.0/token"),
                    postData);

            if (response.isSuccess()) {
                JSONObject j = response.json();
                String streamLabsAccessToken = j.getString("access_token");
                transaction.setProperty("streamlabsaccesstoken", streamLabsAccessToken);

                System.out.println("PhantomBot StreamLabs setup done, PhantomBot will exit.");
                changed = true;
            } else if (response.responseCode() == HttpResponseStatus.BAD_REQUEST) {
                JSONObject e = response.json();
